...
class OBDReader:
    ...
    def get_snapshot(self) -> dict:
        ...
        return {
            "connected": True,
            "rpm": round(rpm.magnitude) if rpm is not None else None,
            "speed_kmh": round(speed.magnitude) if speed is not None else None,
            "engine_temp_c": round(coolant_temp.magnitude) if coolant_temp is not None else None,
            "fuel_percent": round(fuel_level.magnitude) if fuel_level is not None else None,
            "throttle_percent": round(throttle.magnitude) if throttle is not None else None,
            "battery_v": round(voltage.magnitude, 1) if voltage is not None else None,
            "intake_temp_c": round(intake_temp.magnitude) if intake_temp is not None else None,
            "engine_load_percent": round(engine_load.magnitude) if engine_load is not None else None,
            "maf_gs": round(maf.magnitude, 1) if maf is not None else None,
            "o2_voltage_v": round(o2_b1s1.magnitude, 2) if o2_b1s1 is not None else None,
            "fuel_trim_short_percent": round(fuel_trim_short.magnitude, 1) if fuel_trim_short is not None else None,
            "fuel_trim_long_percent": round(fuel_trim_long.magnitude, 1) if fuel_trim_long is not None else None,
            "dtc_codes": dtc_list,
            "mil_on": mil_on,
            "timestamp": time.time(),
        }
